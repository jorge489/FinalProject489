...
class ClientApp(FlaskView):
    def __init__(self, mqtt_host_address, db_host_address,modbus_host_address):
    ...
    def _init_mqtt(self):
        ...
        self.mqttc.subscribe("FWH/2311/Micro850-7.ie.ncsu.edu/Tomatoe", 0)
    ...
    def _enabled_disable_system(self, enable):
        mqtt_topic = "FWH/2311/Micro850-7.ie.ncsu.edu/Tomatoe"
        dataObj={}
        dataObj["Start_Stop"] = enable
        modbus_request = self.modbus_client.read_holding_registers(address=0, count=1)
        self.total_value = modbus_request.registers[0]
        modbus_request = self.modbus_client.read_holding_registers(address=2, count=1)
        self.bad_value = modbus_request.registers[0]  # Value from address 1
        dataObj["Good_Tomatoes"]=self.total_value - self.bad_value
        dataObj["Bad_Tomatoes"]=self.bad_value
    ...
    def _queryDB(self):
        qCmd = """SELECT t.* FROM public."TomatoCounter" t LIMIT 100 """
...
db_host = "postgresql://smartmanufacturing_example3_user:snQ5LnsaayC0VuZXqaZ2l4ftftgc2XtQ@dpg-cspsqn1u0jms73fk3h4g-a.oregon-postgres.render.com/smartmanufacturing_example3"
plc_host = "10.76.152.227"
app_client = ClientApp(mqtt_host, db_host,plc_host)
